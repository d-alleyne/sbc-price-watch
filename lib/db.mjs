 ...
 *
 * `CREATE TABLE IF NOT EXISTS` above is a no-op on an existing table, so it does NOT add columns to
 * one — a database predating a schema change would keep its old shape and every insert naming the
 * new column would fail. Checking `table_info` and issuing `ALTER TABLE` is the only way the store
 * survives a schema change, and losing it would mean losing the accumulated history that change
...
function migrate(db) {
  ...
  if (!columns.has('image_url')) {
    db.exec(`ALTER TABLE observations ADD COLUMN image_url TEXT`);
  }
}
  ...
  const stmt = db.prepare(`
    INSERT INTO observations
      (run_id, merchant, product_ext_id, variant_ext_id, handle, title, variant_title,
       vendor, product_type, sku, price, compare_at_price, currency, available, image_url, observed_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  `);
