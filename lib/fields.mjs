...
/** Money: present, numeric, non-negative. Zero is invalid for `price` — a variant you cannot buy at
 *  the listed number is unusable data, and observed cases are pre-sale placeholders rather than
 *  genuinely free goods. Zero is VALID for `compare_at_price`: see gradeRow. */
 ...
 *
 * `compare_at_price` means "the price this was reduced from", so its absence is not a defect — the
 * item simply is not discounted. Blank counts as ok, otherwise every merchant looks broken on a
 * field working exactly as designed.
 *
 * ZERO also counts as ok here, corrected 2026-07-29 after inspecting the data rather than trusting
 * the grader. 100 litnxt rows carried `compare_at_price: 0` alongside real prices ($229.99,
 * $299.99) — that merchant encodes "no discount" as 0 where others use null. Treating it as invalid
 * understated litnxt by 100 rows and would have produced a confident, wrong claim about their data
 * quality. `price: 0` stays invalid: those were pre-sale placeholders on an unpriced product, which
 * is genuinely unusable.
 */
export function gradeRow(row, tally) {
  const g = {
    price: checkMoney(row.price),
    compare_at_price: isBlank(row.compare_at_price) ? 'ok' : checkMoney(row.compare_at_price, { zeroIsValid: true }),
